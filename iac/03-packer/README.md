# Infrastructure as Code: Building Machine Images
...
- [Infrastructure as Code: Building Machine Images](#infrastructure-as-code-building-machine-images)
...
This example is covered in the [Network Resources](https://ibm.github.io/cloud-enterprise-examples/iac-resources/network) page of the Infrastructure as Code pattern. Refer to that page to know how to use it and execute it.
...
- [Install the IBM Cloud CLI Plugins](https://ibm.github.io/cloud-enterprise-examples/iac/setup-environment#ibm-cloud-cli-plugins) `infrastructure-service`
...
1. [Download & Install Packer](https://www.packer.io/downloads/) for your operative system and architecture. On Mac OS X you can use the `brew` utility like this and validate the installation checking the installed version:
...
2. Create the file `variables.json` file with values for the variables, this is an example with the required and optional variables:
...
3. Modify the `workspace.json` file to change the values of the variables `project_name` and `environment`, currently `iac-network-test-OWNER` and `dev` respectively. It's recommended to replace `OWNER` by your username or user Id to avoid name collisions. It will fail if the word `OWNER` (uppercase) is used.
