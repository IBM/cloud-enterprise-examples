...
resource "ibm_is_subnet" "iac_app_subnet" {
  ...
  public_gateway  = ibm_is_public_gateway.iac_app_public_gateway.id
}
...
resource "ibm_is_public_gateway" "iac_app_public_gateway" {
    name = "${var.project_name}-${var.environment}-public-gw"
    vpc = ibm_is_vpc.iac_app_vpc.id
    zone = "us-south-1"
}
